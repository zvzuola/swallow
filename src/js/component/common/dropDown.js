...
var DropDown = SourceComponent.extend({
    ...
    toggle: function(open) {
        if(this.data.disabled)
            return;
        open = typeof open === 'undefined' ? !this.data.open : !!open;
        this.$update('open', open); 
        if (open) {
            var h = document.body.clientHeight,
                hdRect = this.$refs.hd.getBoundingClientRect(),
                bdRect = this.$refs.bd.getBoundingClientRect();
            this.data.bodyDirection = h < hdRect.bottom + bdRect.bottom - bdRect.top ? 'up' : 'down';
        }
